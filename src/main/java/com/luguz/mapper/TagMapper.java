...
public interface TagMapper extends BaseMapper<Tag> {
    @Results(value = {
            ...
            @Result(property = "blogsNum", column = "ta_id", many = @Many(select = "com.luguz.mapper.TagMapper.getBlogNumByTag",
    ...
    @Results(value = {
            @Result(id = true, property = "taId", column = "ta_id"),
            @Result(property = "blogs", column = "ta_id", many = @Many(select = "com.luguz.mapper.TagMapper.findBlogByTagPublished",
                    fetchType = FetchType.DEFAULT)),
    })
    @Select("select * from t_tag")
